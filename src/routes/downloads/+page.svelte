<script lang="ts">
  ...
  import { history, playlistGroupedHistory, historyStats, formatDuration, isPlaylistGroup, type FilterType, type SortType, type HistoryItem, type HistoryPlaylistGroup } from '$lib/stores/history';
  import { isAndroid, openFileOnAndroid, openFolderOnAndroid } from '$lib/utils/android';
  import { settings } from '$lib/stores/settings';
  import { activeDownloads as queueActiveDownloads, queue, isQueuePaused, pendingDownloadsCount, groupedDownloads } from '$lib/stores/queue';
  ...
  // Track files that no longer exist
  ...
  async function checkFileExists(id: string, filePath: string | undefined) {
    ...
    try {
      await stat(filePath);
      // File exists, ensure it's not in missing set
  ...
  // Check if item's file is missing
  ...
  // Track extracted colors for thumbnail theming
  let itemColors = $state<Map<string, RGB>>(new Map());
  
  // Extract color from thumbnail and cache it
  ...
  // Get inline style for item with extracted color
  ...
  // Track collapsed playlist groups (expanded by default) - for queue
  let collapsedPlaylists = $state<Set<string>>(new Set());
  
  // Track collapsed playlist groups for history (collapsed by default)
  ...
  // Active downloads from queue store (filtered by search)
  ...
  // Grouped downloads with search filter applied
  ...
  // Fix duration for items with 0:00 by reading from file metadata (desktop only)
  onMount(() => {
    if (isAndroid()) return; // ffprobe not available on Android
    
    // Find items with 0 duration that have a file path
    ...
    // Fix durations in background (don't block UI)
    (async () => {
      for (const item of itemsToFix) {
        try {
          ...
          // File might not exist anymore, or ffprobe not available
        }
      }
    })();
  });
  ...
  {#if activeDownloads.length > 0}
    ...
    <div class="active-downloads">
      ...
      {#each grouped.groups as group (group.playlistId)}
        ...
        <div class="playlist-group" class:collapsed={!isExpanded}>
          ...
          <div class="playlist-header-row">
            <button class="playlist-header" onclick={() => togglePlaylistExpanded(group.playlistId)}>
          ...
          {#if isExpanded}
            <div class="playlist-items">
              {#each group.items as download (download.id)}
                ...
                <div class="active-item playlist-item" class:paused={download.status === 'paused'} style="--progress: {displayProgress}%; {getItemColorStyle(download.id)}">
                  ...
                  <div class="active-content">
                    ...
                    <div class="active-info">
                      ...
                      <span class="active-meta">
                        ...
                        {:else}
                          <span class="status-message">{download.statusMessage || $t('downloads.status.downloading')}</span>
                    ...
                    <button 
                      class="cancel-btn" 
                      onclick={(e) => { e.stopPropagation(); queue.cancel(download.id); }}
      ...
      {#each grouped.singles as download (download.id)}
        {@const displayProgress = Math.max(0, Math.round(download.progress))}
        {@const isPending = download.status === 'pending' || download.status === 'paused'}
        <div class="active-item" class:paused={download.status === 'paused'} style="--progress: {displayProgress}%; {getItemColorStyle(download.id)}">
                    ...
                    >
                      <div class="card-thumbnail">
                        {#if subItem.thumbnail}
                          <img src={subItem.thumbnail} alt="" onload={() => { extractItemColor(subItem.id, subItem.thumbnail); checkFileExists(subItem.id, subItem.filePath); }} />
                        {:else}
                        ...
                        {#if hoveredItemId === subItem.id && !fileMissing}
                          <div class="card-overlay">
                            <button 
                              class="play-overlay"
                              onclick={(e) => { e.stopPropagation(); handlePlayFile(e, subItem.filePath); }}
                            ...
                            <div class="card-actions-bar">
                              <button 
                                ...
                                onclick={(e) => { e.stopPropagation(); handleOpenFile(e, subItem.filePath); }}
                              >
                              ...
                              <button 
                                ...
                                onclick={(e) => { e.stopPropagation(); handleRedownload(e, subItem.url); }}
                              >
                              ...
                              <button 
                                ...
                                onclick={(e) => { e.stopPropagation(); handleOpenLink(e, subItem.url); }}
                              >
                              ...
                              <button 
                                ...
                                onclick={(e) => { e.stopPropagation(); handleDelete(e, subItem.id); }}
                              >
                                <Icon name="trash" size={14} />
                              </button>
                            </div>
                  ...
                  >
                    <div class="card-thumbnail">
                      {#if singleItem.thumbnail}
                        <img src={singleItem.thumbnail} alt="" onload={() => { extractItemColor(singleItem.id, singleItem.thumbnail); checkFileExists(singleItem.id, singleItem.filePath); }} />
                      {:else}
            ...
            {#each group.items as item}
              {#if isPlaylistGroup(item)}
                ...
                <div class="playlist-group" class:collapsed={!isExpanded}>
                  ...
                  <div class="playlist-header-row" onclick={() => toggleHistoryPlaylistExpanded(playlistGroup.playlistId)}>
                    <div class="col-thumb">
                    ...
                    <div class="playlist-items">
                      {#each playlistGroup.items as subItem (subItem.id)}
                        ...
                        >
                          <div class="col-thumb">
                            {#if subItem.thumbnail}
                              <img src={subItem.thumbnail} alt="" class="thumbnail" onload={() => { extractItemColor(subItem.id, subItem.thumbnail); checkFileExists(subItem.id, subItem.filePath); }} />
                            {:else}
              ...
              {:else}
                ...
                >
                  <div class="col-thumb">
                    {#if singleItem.thumbnail}
                      <img src={singleItem.thumbnail} alt="" class="thumbnail" onload={() => { extractItemColor(singleItem.id, singleItem.thumbnail); checkFileExists(singleItem.id, singleItem.filePath); }} />
                    {:else}
                      <div class="thumbnail-placeholder">
...
<style>
  ...
  .progress-bg {
    ...
    background: linear-gradient(
      ...
      var(--item-color-alpha-light, var(--accent-alpha-light, rgba(99, 102, 241, 0.15))) calc(var(--progress) - 5%),
      var(--item-color-alpha-lighter, var(--accent-alpha-lighter, rgba(99, 102, 241, 0.08))) var(--progress),
