<script lang="ts">
  ...
  // Get window ID from URL immediately - this is our identity
  ...
  // Detect if URL is from YouTube Music
  ...
  // UI settings
  let cornerDismiss = $state(false); // Use corner X instead of full dismiss button
  
  // Computed accent color with alpha
  ...
  // Close THIS notification window - use our window ID directly
  ...
  // Handle download action - start download immediately
  async function handleDownload(mode: 'auto' | 'audio' = 'auto') {
    isDownloading = true;
    
    try {
      // Pass metadata along with URL so main app doesn't need to re-fetch
      await invoke('notification_action', { 
        ...
        metadata: {
          ...
          downloadMode: mode, // Pass download mode for YTM dual buttons
          isPlaylist: isPlaylist, // Pass playlist flag to main app
        }
      });
    } catch (e) {
  ...
  onMount(() => {
    // Initialize async
    (async () => {
      // Load background settings from store
      try {
        const store = await load('settings.json', { autoSave: false, defaults: {} });
        
        // Always load accent color
      ...
      // Check if marquee is needed after render
      checkMarquee();
...
<div 
  ...
  onmouseleave={() => { isHovered = false; scheduleAutoClose(); }}
>
  ...
  <div class="actions-compact">
      ...
      >
        ...
        {:else}
          <svg viewBox="0 0 24 24" fill="none" width="18" height="18">
            <path d="M4 6H20M4 10H20M4 14H14M4 18H14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
    ...
    {:else if isYouTubeMusic}
      ...
      >
        ...
        {:else}
          <svg viewBox="0 0 24 24" fill="none" width="16" height="16">
            <path d="M4 8V16C4 17.1046 4.89543 18 6 18H18C19.1046 18 20 17.1046 20 16V8C20 6.89543 19.1046 6 18 6H6C4.89543 6 4 6.89543 4 8Z" stroke="currentColor" stroke-width="2"/>
    ...
    {:else}
      <div class="thumb placeholder">
        <svg viewBox="0 0 24 24" fill="none" width="20" height="20">
          <path d="M12 3V16M12 16L16 11.625M12 16L8 11.625" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path opacity="0.5" d="M3 15C3 17.828 3 19.243 3.879 20.121C4.757 21 6.172 21 9 21H15C17.828 21 19.243 21 20.121 20.121C21 19.243 21 17.828 21 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </div>
    {/if}
...
<style>
  ...
  .close-x:hover { color: white; background: rgba(255,255,255,0.1); }
