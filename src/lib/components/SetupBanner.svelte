<script lang="ts">
  ...
  function getStatus(depId: DependencyName) {
    switch (depId) {
      case 'ytdlp': return $deps.ytdlp;
      case 'ffmpeg': return $deps.ffmpeg;
      case 'aria2': return $deps.aria2;
    }
  }
  
  // On Android, dependencies are embedded - no setup needed
  // On desktop, check if any required dependency is missing
  let hasRequiredMissing = $derived(
    isAndroid() ? false : !$deps.ytdlp?.installed
  );
  ...
  <div class="setup-content">
    ...
    {:else if $deps.error}
      <div class="setup-error">
        ...
        <button class="retry-btn" onclick={() => deps.clearError()}>
          Dismiss
        </button>
      </div>
    {:else}
