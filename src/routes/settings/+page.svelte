<script lang="ts">
  ...
  // Define setting items with search metadata
  const settingItems = {
    language: { section: 'general', keywords: ['language', 'язык', 'idioma', 'locale'] },
    startOnBoot: { section: 'general', keywords: ['start', 'boot', 'startup', 'autostart', 'запуск'] },
    ...
    notificationsEnabled: { section: 'notifications', keywords: ['notification', 'popup', 'alert', 'уведомление'] },
    notificationPosition: { section: 'notifications', keywords: ['position', 'corner', 'location', 'позиция'] },
    notificationMonitor: { section: 'notifications', keywords: ['monitor', 'screen', 'display', 'монитор'] },
    compactNotifications: { section: 'notifications', keywords: ['compact', 'small', 'mini', 'icon', 'компактный'] },
    notificationFancyBackground: { section: 'notifications', keywords: ['fancy', 'background', 'blur', 'transparent', 'glass', 'фон', 'прозрачный'] },
    defaultProcessor: { section: 'processing', keywords: ['processor', 'backend', 'cobalt', 'yt-dlp', 'lux', 'auto', 'download'] },
    downloadPath: { section: 'downloads', keywords: ['download', 'path', 'folder', 'directory', 'save', 'location', 'путь', 'папка'] },
    useAudioPath: { section: 'downloads', keywords: ['audio', 'music', 'separate', 'folder', 'path', 'аудио', 'музыка'] },
    usePlaylistFolders: { section: 'downloads', keywords: ['playlist', 'folder', 'subfolder', 'organize', 'плейлист', 'папка'] },
    ...
    accentColor: { section: 'app', keywords: ['accent', 'color', 'theme', 'buttons', 'highlight', 'акцент', 'цвет', 'тема'] },
    toastPosition: { section: 'app', keywords: ['toast', 'position', 'corner', 'location', 'in-app', 'notification', 'внутри', 'позиция'] },
    disableAnimations: { section: 'app', keywords: ['animation', 'disable', 'motion', 'reduce', 'анимация'] },
    sizeUnit: { section: 'app', keywords: ['size', 'unit', 'kb', 'mb', 'binary', 'decimal', 'bytes', 'размер'] },
  ...
  function matchesSearch(settingKey: string): boolean {
    ...
    return item.keywords.some(kw => kw.includes(query)) || 
           $t(`settings.${item.section}.${settingKey === 'language' ? 'language' : settingKey}`).toLowerCase().includes(query);
  }

  // Check if any setting in a section matches
  ...
  // Helper function to convert file path to URL (data URL on Android, asset URL on desktop)
  async function filePathToUrl(filePath: string, mimeType: string): Promise<string> {
    if (isAndroid()) {
      // On Android, asset:// protocol doesn't work reliably, so we read the file and convert to data URL
  ...
  async function pickBackgroundVideo() {
    ...
    if (result) {
      ...
      const mimeType = ext === 'webm' ? 'video/webm' : ext === 'mkv' ? 'video/x-matroska' : ext === 'mov' ? 'video/quicktime' : ext === 'avi' ? 'video/x-msvideo' : 'video/mp4';
  ...
  async function pickBackgroundImage() {
    ...
    if (result) {
      ...
      const mimeType = ext === 'jpg' || ext === 'jpeg' ? 'image/jpeg' : ext === 'gif' ? 'image/gif' : ext === 'webp' ? 'image/webp' : ext === 'bmp' ? 'image/bmp' : 'image/png';
      const assetUrl = await filePathToUrl(filePath, mimeType);
  ...
  // Background type options - acrylic only on desktop
  ...
  // Size unit options - reactive to locale changes
  ...
  // Close behavior options - reactive to locale changes
  ...
  // Toast position options (in-app notifications)
  ...
  // Undo handlers - reset to default
...
<div class="page">
  ...
  <div class="settings-content">
        ...
        <div class="setting-row">
          ...
          <div class="setting-controls">
            {#if $locale !== defaultSettings.language}
              <button class="undo-btn" onclick={undoLanguage} use:tooltip={$t('settings.app.resetToDefault')}>
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.general.closeBehaviorDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.closeBehavior !== defaultSettings.closeBehavior}
              <button class="undo-btn" onclick={() => updateSetting('closeBehavior', defaultSettings.closeBehavior)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
    ...
    <section class="settings-section">
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          ...
          <button 
            class="undo-btn" 
            class:hidden={$settings.notificationsEnabled === defaultSettings.notificationsEnabled}
            onclick={() => updateSetting('notificationsEnabled', defaultSettings.notificationsEnabled)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.notifications.positionDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.notificationPosition !== defaultSettings.notificationPosition}
              <button class="undo-btn" onclick={() => updateSetting('notificationPosition', defaultSettings.notificationPosition)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.notifications.monitorDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.notificationMonitor !== defaultSettings.notificationMonitor}
              <button class="undo-btn" onclick={() => updateSetting('notificationMonitor', defaultSettings.notificationMonitor)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <div style="width: 180px;">
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          ...
          <button 
            class="undo-btn" 
            class:hidden={$settings.compactNotifications === defaultSettings.compactNotifications}
            onclick={() => updateSetting('compactNotifications', defaultSettings.compactNotifications)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          <Checkbox 
            ...
            onchange={(checked: boolean) => updateSetting('notificationFancyBackground', checked)}
          />
          <button class="info-btn" use:tooltip={$t('settings.notifications.fancyBackgroundTooltip')}>
            <Icon name="info" size={18} />
          </button>
          <button 
            class="undo-btn" 
            class:hidden={$settings.notificationFancyBackground === defaultSettings.notificationFancyBackground}
            onclick={() => updateSetting('notificationFancyBackground', defaultSettings.notificationFancyBackground)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          ...
          />
          <button class="info-btn" use:tooltip={$t('settings.notifications.cornerDismissTooltip')}>
            <Icon name="info" size={18} />
          </button>
          <button 
            class="undo-btn" 
            class:hidden={$settings.notificationCornerDismiss === defaultSettings.notificationCornerDismiss}
            onclick={() => updateSetting('notificationCornerDismiss', defaultSettings.notificationCornerDismiss)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.notifications.offsetDescription')}</span>
          </div>
          <div class="slider-with-value">
            {#if $settings.notificationOffset !== defaultSettings.notificationOffset}
              <button class="undo-btn" onclick={() => updateSetting('notificationOffset', defaultSettings.notificationOffset)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <input 
              ...
              oninput={(e) => updateSetting('notificationOffset', parseInt((e.target as HTMLInputElement).value))}
            />
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.processing.defaultProcessorDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.defaultProcessor !== defaultSettings.defaultProcessor}
              <button class="undo-btn" onclick={() => updateSetting('defaultProcessor', defaultSettings.defaultProcessor)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <div style="width: 180px;">
    ...
    <section class="settings-section">
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.downloads.downloadPathDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.downloadPath !== defaultSettings.downloadPath}
              <button class="undo-btn" onclick={() => updateSetting('downloadPath', defaultSettings.downloadPath)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <button class="path-btn" onclick={pickDownloadPath}>
              <Icon name="folder" size={16} />
              <span class="path-text">{$settings.downloadPath || $t('settings.downloads.defaultPath')}</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="setting-sub-row">
          ...
          <div class="setting-controls">
            <button class="path-btn" onclick={pickAudioPath}>
              <Icon name="folder" size={16} />
              <span class="path-text">{$settings.audioPath || $t('settings.downloads.defaultPath')}</span>
            </button>
          </div>
        </div>
        {/if}
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          ...
          />
          <button class="info-btn" use:tooltip={$t('settings.downloads.usePlaylistFoldersTooltip')}>
            <Icon name="info" size={18} />
          </button>
          <button 
            class="undo-btn" 
            class:hidden={$settings.usePlaylistFolders === defaultSettings.usePlaylistFolders}
            onclick={() => updateSetting('usePlaylistFolders', defaultSettings.usePlaylistFolders)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-with-info">
          ...
          />
          <button class="info-btn" use:tooltip={$t('settings.downloads.youtubeMusicAudioOnlyTooltip')}>
            <Icon name="info" size={18} />
          </button>
          <button 
            class="undo-btn" 
            class:hidden={$settings.youtubeMusicAudioOnly === defaultSettings.youtubeMusicAudioOnly}
            onclick={() => updateSetting('youtubeMusicAudioOnly', defaultSettings.youtubeMusicAudioOnly)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.downloads.concurrentDownloadsDescription')}</span>
          </div>
          <div class="slider-with-value">
            {#if $settings.concurrentDownloads !== defaultSettings.concurrentDownloads}
              <button class="undo-btn" onclick={() => updateSetting('concurrentDownloads', defaultSettings.concurrentDownloads)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <input 
              ...
              oninput={(e) => updateSetting('concurrentDownloads', parseInt((e.target as HTMLInputElement).value))}
            />
      ...
      <div class="setting-item">
        ...
        <div class="setting-sub-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.app.backgroundVideoUrlDescription')}</span>
          </div>
          <div class="input-with-actions">
            {#if $settings.backgroundVideo !== defaultSettings.backgroundVideo}
            <button 
              class="undo-btn" 
              onclick={() => { backgroundVideoInput = defaultSettings.backgroundVideo; handleBackgroundVideoChange(defaultSettings.backgroundVideo); }}
              use:tooltip={$t('settings.app.resetToDefault')}
            >
        ...
        <div class="setting-sub-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.app.backgroundImageUrlDescription')}</span>
          </div>
          <div class="input-with-actions">
            {#if $settings.backgroundImage !== defaultSettings.backgroundImage}
            <button 
              class="undo-btn" 
              onclick={() => { backgroundImageInput = defaultSettings.backgroundImage; handleBackgroundImageChange(defaultSettings.backgroundImage); }}
              use:tooltip={$t('settings.app.resetToDefault')}
            >
              <Icon name="undo" size={16} />
            </button>
        ...
        <div class="setting-sub-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.app.backgroundBlurDescription')}</span>
          </div>
          <div class="slider-with-value">
            <input 
              type="range" 
              ...
              oninput={(e) => handleBackgroundBlurChange(parseInt((e.target as HTMLInputElement).value))}
            />
          ...
          <div class="setting-control-group">
            ...
            <div class="color-picker-group">
              ...
              <input 
                ...
                value={$settings.accentColor}
                disabled={$settings.useSystemAccent}
                oninput={(e) => handleAccentColorChange((e.currentTarget as HTMLInputElement).value)}
                placeholder="#6366F1"
              />
            </div>
          </div>
      ...
      <div class="setting-item">
        <div class="setting-with-undo">
          ...
          <button 
            ...
            onclick={() => updateSetting('disableAnimations', defaultSettings.disableAnimations)} 
            use:tooltip={$t('settings.app.resetToDefault')}
          >
            <Icon name="undo" size={18} />
          </button>
      ...
      <div class="setting-item">
        <div class="setting-row">
          <div class="setting-label-group">
            ...
            <span class="setting-description">{$t('settings.notifications.toastPositionDescription')}</span>
          </div>
          <div class="setting-controls">
            {#if $settings.toastPosition !== defaultSettings.toastPosition}
              <button class="undo-btn" onclick={() => updateSetting('toastPosition', defaultSettings.toastPosition)} use:tooltip={$t('settings.app.resetToDefault')}>
                <Icon name="undo" size={14} />
              </button>
            {/if}
            <div style="width: 180px;">
      ...
      <div class="setting-item">
        <div class="setting-with-undo">
          <Checkbox 
            ...
            onchange={() => { updateSetting('showHistoryStats', !$settings.showHistoryStats); }}
          />
          <button 
            class="undo-btn" 
      ...
      <div class="setting-item">
        <div class="setting-with-undo">
          <Checkbox 
            ...
            onchange={() => { updateSetting('thumbnailTheming', !$settings.thumbnailTheming); }}
          />
          <button 
            class="undo-btn" 
    ...
    <section class="settings-section">
      ...
      <div class="setting-item">
        ...
        {#if $deps.installingDeps.has('quickjs') && $deps.installProgressMap.get('quickjs')}
          <div class="dep-progress">
            <div class="dep-progress-bar" style="width: {$deps.installProgressMap.get('quickjs')?.progress ?? 0}%"></div>
          </div>
        {/if}
      </div>
    ...
    <section class="settings-section">
      ...
      <!-- Import Message -->
      {#if importMessage}
        <p class="import-message" class:success={importMessage.type === 'success'} class:error={importMessage.type === 'error'}>
          {importMessage.text}
        </p>
      {/if}
    </section>
